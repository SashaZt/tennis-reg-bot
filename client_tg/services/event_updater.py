...
#     """Простой обновлятор событий каждый час"""
...
#         """Получить все активные события от сегодня и вперед"""
...
#                 """SELECT * FROM events
#                    WHERE is_active = TRUE AND event_date >= ?
...
#                 for row in rows:
#                     event = Event(
#                         id=row[0], title=row[1], event_date=row[2], event_time=row[3],
#                         location=row[4], max_participants=row[5], price=row[6],
#                         created_by=row[7], is_active=row[8], created_at=row[9],
...
#         """Обновить сообщение события в группе"""
#         try:
...
#             logger.info(f"✅ Событие {event.id} ({event.title}) обновлено")
#             return True
...
#                 logger.debug(f"📋 Событие {event.id} не изменилось")
#                 return True
#             elif "message to edit not found" in error_msg:
#                 logger.warning(f"⚠️ Сообщение события {event.id} не найдено")
#                 return False
#             else:
#                 logger.error(f"❌ Ошибка обновления {event.id}: {e}")
#                 return False

#     async def update_all_events(self):
#         """Обновить все события"""
#         logger.info("🔄 Начинаю обновление всех событий...")

#         events = await self.get_future_events()

#         if not events:
#             logger.info("ℹ️ Нет событий для обновления")
#             return
...
#         logger.info(f"📊 Обновлено событий: {updated}/{len(events)}")

#     async def hourly_update_loop(self):
#         """Цикл обновления каждый час"""
#         self.is_running = True
#         logger.info("🚀 Запущено автообновление событий каждый час")
